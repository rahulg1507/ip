...
Generate one interactive HTML page containing every changed file as a side-by-side before/after diff. Fold long unchanged runs, highlight changed words within modified lines, filter files, and include collapsed panels for unchanged files.
...
Check that the page exists and that the generator's summary reports the expected changed-file count. Open the generated HTML file in a browser or inspect its rendered page only when the user asks for a visual review.
